...
class IncrementalProduct(Model):
    ...
    def __init__(self, globalSpec):
        ...
        ###################
        from twtl.dfa import setDFAType, DFAType, Op, setOptimizationFlag
        
        from twtl.twtl import translate
        # self.buchi = translate('[H^2 A]^[0, 4] | [H^2 B]^[2, 5]',
        #                        kind=DFAType.Infinity, norm=True)[1]
        self.buchi = translate(self.globalSpec,
                               kind=DFAType.Infinity, norm=True)[1]

        self.buchi.visualize()
        ###################
